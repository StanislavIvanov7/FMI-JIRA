...
void Task::setGrade(double newGrade, const User* changedBy, const Date& timestamp) {
	...
	changeHistory.emplace_back(changedBy, std::string(TaskConstants::FIELD_GRADE), std::to_string(grade), std::to_string(newGrade), timestamp);
	grade = newGrade;
}
